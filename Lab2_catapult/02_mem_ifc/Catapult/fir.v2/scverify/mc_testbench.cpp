...
// Generated date: Sun Mar 24 17:12:05 CST 2024
//
// ----------------------------------------------------------------------------
// 
// -------------------------------------
...
void testbench::exec_run(ac_channel<ac_int<8, true> > &input, ac_int<8, true> coeffs[32][8], ac_channel<ac_int<5, false> > &coeff_addr, ac_channel<ac_int<8, true> > &output) {
   return mc_testbench::exec_run(input, coeffs, coeff_addr, output);
}
#endif
...
void mc_testbench::capture_coeffs( ac_int<8, true > coeffs[32][8])
{
   ...
   if (_capture_coeffs && !testbench::coeffs_ignore)
   {
      ...
      mgc_sysc_ver_array1D<ac_int<8, true >,256> coeffs_tmp;
      int coeffs_linear_idx = 0;
      for (int coeffs_idx_0 = 0; coeffs_idx_0 < 32; ++coeffs_idx_0)
         for (int coeffs_idx_1 = 0; coeffs_idx_1 < 8; ++coeffs_idx_1)
...
void mc_testbench::capture_coeff_addr( ac_channel<ac_int<5, false > > &coeff_addr)
{
   ...
   {
      if (!coeff_addr_pointer_set) {
         coeff_addr_pointer = &(coeff_addr);
         coeff_addr_pointer_set = true;
      } else if (coeff_addr_pointer != &(coeff_addr) && remaining_ccs_coeff_addr == 0) {
         std::ostringstream msg; msg.str("");
         msg << "Interface coeff_addr source switched to a different object in testbench with "
             << remaining_ccs_coeff_addr << " values remaining in input fifo. No data mismatch detected." <<" @ " << sc_time_stamp();
         SC_REPORT_INFO("User testbench", msg.str().c_str());
         coeff_addr_pointer = &(coeff_addr);
      } else if (coeff_addr_pointer != &(coeff_addr) && remaining_ccs_coeff_addr > 0 && remaining_ccs_coeff_addr != coeff_addr.ac_channel<ac_int<5, false > >::debug_size()) {
         std::ostringstream msg; msg.str("");
         msg << "Interface coeff_addr source switched to a different object in testbench with "
             << remaining_ccs_coeff_addr << " values remaining in input fifo. Simulation mismatch likely. Check golden vs DUT comparison values for errors." <<" @ " << sc_time_stamp();
         SC_REPORT_WARNING("User testbench", msg.str().c_str());
         _channel_mismatch = true;
      }
...
void mc_testbench::wait_on_input_required()
{
   ...
   while (atleast_one_active_input) {
      ...
      if (average_period > SC_ZERO_TIME && sc_time_stamp() != timeout)
         wait(average_period * 10, ccs_input->ok_to_put() | ccs_coeffs->ok_to_put() | ccs_coeff_addr->ok_to_put());
      else
         wait(ccs_input->ok_to_put() | ccs_coeffs->ok_to_put() | ccs_coeff_addr->ok_to_put());
      that->cpp_testbench_active.write(true);
      if (timed_out()) {
...
void mc_testbench::capture_IN(ac_channel<ac_int<8, true> > &input, ac_int<8, true> coeffs[32][8], ac_channel<ac_int<5, false> > &coeff_addr, ac_channel<ac_int<8, true> > &output)
{
...
void mc_testbench::capture_OUT(ac_channel<ac_int<8, true> > &input, ac_int<8, true> coeffs[32][8], ac_channel<ac_int<5, false> > &coeff_addr, ac_channel<ac_int<8, true> > &output)
{
...
void mc_testbench::exec_run(ac_channel<ac_int<8, true> > &input, ac_int<8, true> coeffs[32][8], ac_channel<ac_int<5, false> > &coeff_addr, ac_channel<ac_int<8, true> > &output)
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   wait_for_idle_sync();
   capture_IN(input, coeffs, coeff_addr, output);
   fir::run(input, coeffs, coeff_addr, output);
   ...
   capture_OUT(input, coeffs, coeff_addr, output);
   #else
   #endif
}
// ============================================
